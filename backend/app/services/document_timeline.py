...
def infer_document_date(doc: Document, entities: list[ExtractedEntity]) -> Optional[date]:
    ...
    # Prefer a date entity persisted by document extraction.
    ...
    # Fall back to OCR text when extraction did not persist a date entity.
    if doc.ocr_text:
        parsed = _parse_date(doc.ocr_text)
        if parsed:
            return parsed
...
class DocumentTimelineService:
    ...
    async def timeline(self, patient_id: uuid.UUID) -> dict[str, Any]:
        ...
        for document in documents:
            ...
            group_key = event_date.isoformat() if event_date else "undated"
